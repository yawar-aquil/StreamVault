...
async function main() {
  console.log('🎬 Adding The Boys show with episodes...\n');

  try {
    // Read the JSON file
    const jsonData = readFileSync('C:\\Users\\yawar\\Desktop\\the-boys.json', 'utf-8');
    const episodesData: SeasonData = JSON.parse(jsonData);

    // Use the known TMDB ID for The Boys
    const tmdbId = 76479; // The Boys (2019)
    ...
    let theBoysShow = shows.find(s => s.title.toLowerCase() === 'the boys');

    if (theBoysShow) {
      console.log(`ℹ️  Show already exists: ${theBoysShow.title} (ID: ${theBoysShow.id})`);
      console.log(`📝 Updating show with cast information...\n`);
      
      // Update the show with cast
      theBoysShow = await storage.updateShow(theBoysShow.id, {
        ...theBoysShow,
        cast: cast,
      });
      ...
      // Create the show
      console.log('✨ Creating The Boys show...');
      ...
      const showPayload = {
        ...
        backdrop: showDetails.backdrop_path
          ...
          : '',
        year: parseInt(showDetails.first_air_date?.split('-')[0] || '2019'),
        rating: 'TV-MA',
        ...
        network: showDetails.networks[0]?.name || 'Amazon Prime Video',
        featured: true,
        trending: true,
      };

      theBoysShow = await storage.createShow(showPayload);
      console.log(`✅ Created show: ${theBoysShow.title} (ID: ${theBoysShow.id})\n`);
    }
    ...
    for (const [seasonKey, episodes] of Object.entries(episodesData)) {
      ...
      const existingEpisodes = await storage.getEpisodesByShowId(theBoysShow.id);
    ...
    console.log('\n✅ The Boys show and episodes added successfully!');
