...
async function ensureSkinCategory() {
    ...
    data.badges = data.badges.map((badge: any) => {
        if (badge.name === 'Neon Skin' || badge.name === 'Gold Skin') {
            if (badge.category !== 'skin') {
                console.log(`Updating category for ${badge.name}: ${badge.category} -> skin`);
                badge.category = 'skin';
                updatedCount++;
            }
        }
    ...
    if (updatedCount > 0) {
        ...
        console.log('Badge categories are already correct.');
    }
}
